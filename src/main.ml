...
let convert_variant ~name ~js =
  ...
  | _ -> eprintf "Must specify the name for converter is `convert', not %s" name; exit 2
...
let generate_randomize_json json num =
  ...
  List.range 0 num |>
    List.map ~f:(fun _ ->
                 let open Lazy in
                 Lazy.from_fun (replace json) >>| (S.to_channel Out_channel.stdout)
                )

let main num path () =
  let filename = match Sys.is_file path with
    | `Yes -> path
    | _ -> eprintf "Template must be an file is exists"; exit 1
  in
  let js = S.from_file filename in
  generate_randomize_json js num |> List.map ~f:Lazy.force |> ignore
